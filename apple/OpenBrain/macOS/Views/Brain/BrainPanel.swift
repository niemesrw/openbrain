...
struct BrainPanel: View {
            ...
            ScrollView {
                LazyVStack(spacing: 12) {
                    ...
                    ForEach(viewModel.messages) { message in
                        BrainChatBubble(message: message)
                            .id(message.id)
                    }
    ...
    private var inputBar: some View {
        HStack(spacing: 8) {
            ...
            .buttonStyle(.plain)
            .foregroundStyle(canSend ? Color.purple : Color.gray.opacity(0.3))
            .disabled(!canSend)
...
private struct BrainChatBubble: View {
    ...
    var body: some View {
        HStack {
            ...
            if message.role == .agent { Spacer(minLength: 80) }
        }
        .padding(.horizontal)
    }
}
